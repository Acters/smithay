...
static int bridge_copy(int src_fd, uint32_t src_stride, uint64_t src_mod,
    ...
    sici.pNext = &src_ext;
    sici.tiling = VK_IMAGE_TILING_LINEAR;
    ...
    printf("   src image memoryTypeBits=0x%x size=%zu\n", sreq.memoryTypeBits, (size_t)sreq.size);
...
int main(void) {
    ...
    test_oes_import_one(GBM_FORMAT_XRGB8888, 0ULL, "XR24 LINEAR");
    test_oes_import_one(GBM_FORMAT_XRGB8888, 0x100000000000002ULL, "XR24 X-TILED");
    test_oes_import_one(GBM_FORMAT_ABGR2101010, 0ULL, "AB30 LINEAR");
    for (int t = 0; t < 2; t++) {
