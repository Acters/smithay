// vkbridge_poc — proof of concept for the Vulkan cross-GPU copy bridge:
...
//
// Validates the one untested link in the smithay multigpu fallback plan.
// Build: gcc -O1 -o vkbridge_poc vkbridge_poc.c -lgbm -lEGL -lGLESv2 -lvulkan
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <fcntl.h>
...
static int gles_fill(struct gbm_device *gbm, EGLDisplay dpy, struct gbm_bo *bo,
    ...
    eglMakeCurrent(dpy, EGL_NO_SURFACE, EGL_NO_SURFACE, dpy == nv_dpy ? nv_ctx : in_ctx);
...
static int vk_init(uint32_t want_major, uint32_t want_minor) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        printf("   vk phd %u: %s (render %u:%u)\n", i, p2.properties.deviceName,
               drm.renderMajor, drm.renderMinor);
...
// Test NVIDIA EGL import of an Intel LINEAR dmabuf as GL_TEXTURE_EXTERNAL_OES,
// per the NVIDIA engineer note that EXTERNAL_OES is required for imported
// images (see open-gpu-kernel-modules discussion #243).
static void test_oes_import_one(uint32_t fourcc, uint64_t modifier, const char *name) {
    printf("== OES import: %s (mod 0x%llx)\n", name, (unsigned long long)modifier);
    struct gbm_bo *bo = gbm_bo_create_with_modifiers(in_gbm, 64, 64,
                                                     fourcc, &modifier, 1);
    if (!bo) { printf("   intel alloc failed\n"); return; }
    if (gles_fill(in_gbm, in_dpy, bo, fourcc, 1.f, 0.f, 0.f)) { printf("   fill failed (fbo)\n"); gbm_bo_destroy(bo); return; }
    int bfd = gbm_bo_get_fd(bo);
    EGLAttrib a[20]; int i = 0;
    a[i++] = EGL_WIDTH; a[i++] = 64; a[i++] = EGL_HEIGHT; a[i++] = 64;
    a[i++] = EGL_LINUX_DRM_FOURCC_EXT; a[i++] = (EGLAttrib)fourcc;
    ...
    a[i++] = EGL_DMA_BUF_PLANE0_OFFSET_EXT; a[i++] = 0;
    a[i++] = EGL_DMA_BUF_PLANE0_PITCH_EXT; a[i++] = (EGLAttrib)gbm_bo_get_stride_for_plane(bo, 0);
    a[i++] = EGL_DMA_BUF_PLANE0_MODIFIER_LO_EXT; a[i++] = (EGLAttrib)(modifier & 0xFFFFFFFF);
    a[i++] = EGL_DMA_BUF_PLANE0_MODIFIER_HI_EXT; a[i++] = (EGLAttrib)(modifier >> 32);
    a[i++] = EGL_NONE;
    eglMakeCurrent(nv_dpy, EGL_NO_SURFACE, EGL_NO_SURFACE, nv_ctx);
    EGLImage img = pCreateImage(nv_dpy, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, NULL, a);
    if (!img) { printf("   eglCreateImage: FAILED 0x%x\n", eglGetError()); gbm_bo_destroy(bo); return; }
    printf("   eglCreateImage: OK\n");
    GLuint t;
    glGenTextures(1, &t);
    glBindTexture(GL_TEXTURE_EXTERNAL_OES, t);
    pImageTargetTex(GL_TEXTURE_EXTERNAL_OES, img);
    GLenum e = glGetError();
    printf("   bind EXTERNAL_OES: %s (0x%x)\n", e == GL_NO_ERROR ? "OK" : "FAIL", e);
    // try sampling: draw with it into an RGBA8 FBO via a trivial shader
    if (e == GL_NO_ERROR) {
        GLuint t2, f2;
        glGenTextures(1, &t2); glBindTexture(GL_TEXTURE_2D, t2);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 64, 64, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
        glGenFramebuffers(1, &f2); glBindFramebuffer(GL_FRAMEBUFFER, f2);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, t2, 0);
        const char *vs_s = "attribute vec2 p; varying vec2 uv; void main(){ uv=p*0.5+0.5; gl_Position=vec4(p,0.,1.); }";
        const char *fs_s = "#extension GL_OES_EGL_image_external : enable\nprecision mediump float; varying vec2 uv; uniform samplerExternalOES tex; void main(){ gl_FragColor = texture2D(tex, uv); }";
        GLuint vs = glCreateShader(GL_VERTEX_SHADER), fs = glCreateShader(GL_FRAGMENT_SHADER);
        glShaderSource(vs, 1, &vs_s, NULL); glCompileShader(vs);
        glShaderSource(fs, 1, &fs_s, NULL); glCompileShader(fs);
        GLint ok; char logb[512];
        glGetShaderiv(fs, GL_COMPILE_STATUS, &ok);
        if (!ok) { glGetShaderInfoLog(fs, 512, NULL, logb); printf("   fs: %s\n", logb); return; }
        GLuint prog = glCreateProgram();
        glAttachShader(prog, vs); glAttachShader(prog, fs);
        glBindAttribLocation(prog, 0, "p");
        ...
        uint8_t px[4] = {0};
        glReadPixels(32, 32, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, px);
        printf("   sampled center pixel RGBA = %u %u %u %u %s\n", px[0], px[1], px[2], px[3],
               (px[0] > 200) ? "(IMPORT WORKS!)" : "(broken)");
    }
    gbm_bo_destroy(bo);
}
...
int main(void) {
    ...
    // fill XB30 via vulkan transfer (intel GLES can't render XB30 FBO), then OES import
    {
        uint32_t fourcc = GBM_FORMAT_XBGR2101010;
        uint64_t linear = 0ULL;
        struct gbm_bo *bo = gbm_bo_create_with_modifiers2(in_gbm, 64, 64, fourcc, &linear, 1, 0);
        printf("== OES import: XB30 LINEAR (via vulkan fill)\n");
        if (!bo) { printf("   intel alloc failed\n"); }
        else {
            // render XR24 red into a temp buffer, vulkan-copy it into the XB30 bo
            struct gbm_bo *tmp = gbm_bo_create_with_modifiers2(in_gbm, 64, 64, GBM_FORMAT_XRGB8888, &linear, 1, 0);
            if (gles_fill(in_gbm, in_dpy, tmp, GBM_FORMAT_XRGB8888, 1.f, 0.f, 0.f) == 0) {
                int src_fd = gbm_bo_get_fd(tmp), dst_fd = gbm_bo_get_fd(bo);
                if (bridge_copy(src_fd, gbm_bo_get_stride_for_plane(tmp, 0),
                                gbm_bo_get_modifier(tmp), VK_FORMAT_B8G8R8A8_UNORM,
                                dst_fd, gbm_bo_get_stride_for_plane(bo, 0),
                                VK_FORMAT_A2B10G10R10_UNORM_PACK32) == 0) {
                    printf("   vulkan filled XB30 with red\n");
                } else printf("   vulkan fill failed\n");
            }
            // now OES-import the XB30 bo
            int bfd = gbm_bo_get_fd(bo);
            EGLAttrib a[20]; int i = 0;
            a[i++] = EGL_WIDTH; a[i++] = 64; a[i++] = EGL_HEIGHT; a[i++] = 64;
            a[i++] = EGL_LINUX_DRM_FOURCC_EXT; a[i++] = (EGLAttrib)fourcc;
            ...
            if (!img) { printf("   eglCreateImage: FAILED 0x%x\n", eglGetError()); }
                ...
                if (e == GL_NO_ERROR) {
                    ...
                    glReadPixels(32, 32, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, px);
                    printf("   sampled center pixel RGBA = %u %u %u %u %s\n", px[0], px[1], px[2], px[3],
                           (px[0] > 200) ? "(IMPORT WORKS!)" : "(broken/black)");
                }
            }
            gbm_bo_destroy(tmp);
        }
    ...
    for (int t = 0; t < 2; t++) {
        ...
        uint64_t linear_mod = 0ULL;
        struct gbm_bo *bo = gbm_bo_create_with_modifiers(in_gbm, 64, 64, tests[t].fourcc,
                                          &linear_mod, 1);
        ...
        if (gles_fill(in_gbm, in_dpy, bo, tests[t].fourcc, tests[t].r, tests[t].g, tests[t].b)) continue;
        printf("   GLES rendered solid color into src (on INTEL)\n");
