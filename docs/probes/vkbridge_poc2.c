// vkbridge_poc2 — exploratory variant of the Vulkan cross-GPU copy bridge:
...
//
// Historical experiment; unlike vkbridge_poc.c, this does not compare the
// sampled pixel against an expected value.
// Build: gcc -O1 -Wall -Wextra -o vkbridge_poc2 vkbridge_poc2.c $(pkg-config --cflags --libs gbm egl glesv2 vulkan)
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <fcntl.h>
...
static void test_linear_render_and_external_only(void) {
    printf("== test: render into flags-0 LINEAR BO + external_only query\n");
    uint64_t linear = 0ULL;
    struct gbm_bo *bo = gbm_bo_create_with_modifiers2(nv_gbm, 64, 64, GBM_FORMAT_XRGB8888,
                                                      &linear, 1, 0);
    printf("   flags-0 LINEAR alloc: %s (mod=0x%llx)\n", bo ? "OK" : "FAIL",
           bo ? (unsigned long long)gbm_bo_get_modifier(bo) : 0ULL);
    if (bo) {
        int rc = gles_fill(nv_gbm, nv_dpy, bo, GBM_FORMAT_XRGB8888, 1.f, 0.f, 0.f);
        printf("   gles_fill into LINEAR BO: %s\n", rc == 0 ? "RENDERED OK" : "FAILED");
        gbm_bo_destroy(bo);
    }
    typedef EGLBoolean (EGLAPIENTRYP QMODS)(EGLDisplay, EGLint, EGLint, EGLuint64KHR*, EGLBoolean*, EGLint*);
    QMODS qmods = (QMODS)eglGetProcAddress("eglQueryDmaBufModifiersEXT");
    if (qmods) {
        EGLuint64KHR mods[64]; EGLBoolean eo[64]; EGLint count = 0;
        qmods(nv_dpy, GBM_FORMAT_XRGB8888, 64, mods, eo, &count);
        int found = 0;
        for (int i = 0; i < count; i++)
            if (mods[i] == 0ULL) { printf("   XR24 LINEAR external_only=%d\n", eo[i]); found = 1; }
        if (!found) printf("   XR24 LINEAR not in NVIDIA modifier list\n");
    }
}
...
int main(void) {
    ...
    test_linear_render_and_external_only();
